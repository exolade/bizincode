...
public class CustomerDaoImpl implements AbstractDao{
	...
	public CustomerDaoImpl(EmfProvider provider) {
		try {
	...
	public List<Object> getByAge(int age) {
		Query q = em.createNamedQuery("Customer.findByAge");
	...
	public List<Object> getByAddress(String address) {
		Query q = em.createNamedQuery("Customer.findByAddress");
	...
	public List<Object> getByGender(char gender) {
		Query q = em.createNamedQuery("Customer.findByGender");
	...
	public int deleteByQuery(String query, List<String> tag, List<Object> condition) {
		...
		for (int i = 0; i < result.size(); i++) {
			Customer tmp = (Customer) result.get(i);
			tmp.setProfile(null);
			em.remove(tmp);
		}
		em.getTransaction().commit();
